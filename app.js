...
const STORAGE='ksaNurseCatStatsV2InpatientSurgical';
const letters=['A','B','C','D','E'];
const CATEGORY_PRIORITY={
 'Postoperative & ABCDE':2.0,
 'Bleeding & Shock':1.9,
 'Respiratory / DVT / PE':1.85,
 'Wounds / Drains / Ostomy':1.75,
 'Medications & Antidotes':1.65,
 'Sepsis / Infection / Safety':1.6,
 'Fluids / Electrolytes / Calculations':1.45,
 'Prioritization / SBAR / Delegation':1.4,
 'KFSHRC & Surgical Inpatient':1.15,
 'Preoperative / Discharge / Behavioral':1.0
};
...
function updateHome(){
 ...
 $('caseCount').textContent=window.KSA_QUESTIONS.filter(q=>q.case).length;
 ...
 $('statLevel').textContent=s.answered?'Nível '+levelFromAbility(s.ability??.5):'—';
}
...
function start(mode){
 ...
 if(mode==='errors'){
   ...
   if(!pool.length){alert('Você ainda não possui questões erradas salvas. Faça um CAT ou revisão primeiro.');return;}
   state.length=Math.min(20,pool.length);
 }
...
function pickQuestion(){
 ...
 if(state.mode==='cat'){
   ...
   pool=pool.map(q=>{
     ...
     const relevanceBonus=(weight-1)*.32;
...
function renderQuestion(q){
 ...
 if(q.case){caseBox.textContent=q.case;caseBox.classList.remove('hidden')}else caseBox.classList.add('hidden');
 ...
 q.options.forEach((opt,i)=>{
   ...
   b.addEventListener('click',()=>answer(i,b));options.appendChild(b)
 });
 $('feedback').className='feedback hidden';$('feedback').innerHTML='';$('nextBtn').classList.add('hidden');
}
function answer(choice,button){
 if(state.answered)return;state.answered=true;
 const q=state.current;const correct=choice===q.answer;const buttons=[...document.querySelectorAll('.option')];
 buttons.forEach((b,i)=>{b.disabled=true;if(i===q.answer)b.classList.add('correct');});if(!correct)button.classList.add('wrong');
 const source=window.KSA_SOURCES[q.source]||null;
 const sourceHtml=source?(source.url?`<a href="${source.url}" target="_blank" rel="noopener">Fonte: ${source.label} ↗</a>`:`<span class="muted">Fonte: ${source.label}</span>`):'';
 const feedback=$('feedback');feedback.className='feedback '+(correct?'correct':'wrong');
 feedback.innerHTML=`<h4>${correct?'✓ Correto':'✕ Incorreto — resposta: '+letters[q.answer]}</h4><p>${q.rationale}</p>${sourceHtml}`;
 const entry={id:q.id,category:q.category,difficulty:q.difficulty,correct};state.session.push(entry);
 const s=loadStats();s.answered=(s.answered||0)+1;if(correct)s.correct=(s.correct||0)+1;
 let wrong=new Set(s.wrongIds||[]);if(correct)wrong.delete(q.id);else wrong.add(q.id);s.wrongIds=[...wrong];
 if(state.mode==='cat'){const delta=correct?(.09+.015*q.difficulty):(-.11-.01*q.difficulty);state.ability=clamp(state.ability+delta,0,1);s.ability=state.ability;}
 saveStats(s);
 $('nextBtn').classList.remove('hidden');$('nextBtn').textContent=state.index+1>=state.length?'Ver resultado':'Próxima questão';
}
function finish(){
